  ...
  for (let index in words) {
    ...
    if (word.orientation != "none") {
      ...
      if (!(tempStr in positions)) {
        positions[tempStr] = positionCounter;
        positionCounter += 1;
      }
...
function attemptToInsert(rows, cols, table, weights, verticalCount, totalCount, word, index, usedPositions) {
  ...
  // Horizontal
  for (let i = 0; i < rows; i++) {
    // let horizontalSkip = false; // Flag to skip iteration
    for (let j = 0; j < cols - word.length + 1; j++) {
      ...
      var hasVerticalConflict = usedPositions.some(pos => pos[0] === i && pos[1] ===j); 
      ...
      var positionUsed = usedPositions.some(pos => pos[0] === i && pos[1] === j);
  ...
  for (let i = 0; i < rows - word.length + 1; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      var hasHorizontalConflict = usedPositions.some(pos => pos[0] === i && pos[1] === j);
  ...
  if (bestScore > -1) {
    usedPositions.push([bestI,bestJ]) ;
    ...
    if (availablePositions.length > 0) {
      ...
      return [0, word, index, randomPosition[0], randomPosition[1], 0];
    } else {
      // If no available position at all, return error
      return [-1];
    }
  }
}

function generateTable(table, rows, cols, words, weights, usedPositions) {
  var verticalCount = 0;
  var totalCount = 0;
  ...
  for (let outerIndex in words) {
    ...
    for (let innerIndex in words) {
      if ("answer" in words[innerIndex] && !("startx" in words[innerIndex])) {
        var temp = attemptToInsert(rows, cols, table, weights, verticalCount, totalCount, words[innerIndex].answer, innerIndex, usedPositions);
        if (temp[0] > best[0]) {
          best = temp;
        }
      }
...
function validatePositions(words) {
  for (let index in words) {
    var word = words[index];
    if (word.orientation != "none" && !word.position) {
      console.error("Missing position for word:", word);
      // Take appropriate action, such as regenerating the layout or assigning a default position
    }
  }
}
...
function generateSimpleTable(words) {
  // console.log("Words before layout generation:", words);
  var rows = computeDimension(words, 3);
  var cols = rows;
  var blankTable = initTable(rows, cols);
  var usedPositions = [];
  var table = generateTable(blankTable, rows, cols, words, [0.7, 0.15, 0.1, 0.05], usedPositions);
  var newTable = removeIsolatedWords(table);
  var finalTable = trimTable(newTable);
  assignPositions(finalTable.result);
  // console.log("Words after layout generation:", finalTable.result);
  validatePositions(finalTable.result);
  return finalTable;
}

function generateLayout(words_json, difficulty) {
